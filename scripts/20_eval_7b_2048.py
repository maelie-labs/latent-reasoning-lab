...
"""
20_eval_7b_2048.py
Comprehensive evaluation of DeepSeek-R1-Distill-Qwen-7B across 25 benchmark problems
with UNCONSTRAINED max_new_tokens = 2048 across all recurrent conditions:
...
Saves to data/eval_results_7b_2048.json.
Runs on cuda:1.
"""
...
OUTPUT_FILE = "data/eval_results_7b_2048.json"
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate 7B model with max_new_tokens=2048")
    parser.add_argument("--rerun_discrete", action="store_true", help="Re-run discrete baseline instead of reusing existing 2048-token discrete results")
    args = parser.parse_args()

    print(f"=== Stage: Evaluating 7B Model (Discrete vs Recurrent) with max_new_tokens=2048 on {DEVICE} ===")
    
    tokenizer = AutoTokenizer.from_pretrained(MODEL_ID)
    think_end_id = tokenizer.encode("</think>", add_special_tokens=False)[-1]
    
    print(f"Loading base model {MODEL_ID} in bfloat16 on {DEVICE}...")
    ...
    print(f"Calculated scale factor alpha_7B: {scale_factor:.6f}")
    ...
    print("\n" + "="*90)
    print("CONDITION 1: DISCRETE BASELINE COT (DeepSeek-R1-Distill-Qwen-7B Teacher)")
    print("="*90)
    ...
    print("\n" + "="*110)
    print("7B BENCHMARK EVALUATION SUMMARY TABLE (max_new_tokens=2048)")
    print("="*110)
