#!/usr/bin/env python3
"""
16_collect_traces_14b.py
Self-distillation trace collector for deepseek-ai/DeepSeek-R1-Distill-Qwen-14B in 8-bit.
Generates 100 reasoning traces (<think> ... </think> + final answer) on GSM8K training set.
Saves to data/traces_gsm8k_14b.jsonl.
Runs on cuda:1 (RTX PRO 4500 32GB) with batch size 8.
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description="Collect reasoning traces from DeepSeek-R1-Distill-14B")
    parser.add_argument("--num_samples", type=int, default=100, help="Number of traces to collect")
    parser.add_argument("--batch_size", type=int, default=8, help="Batch size for generation")
    parser.add_argument("--output_file", type=str, default="data/traces_gsm8k_14b.jsonl", help="Output jsonl path")
    parser.add_argument("--max_new_tokens", type=int, default=768, help="Max tokens per generation")
    return parser.parse_args()

def main():
    args = parse_args()
    print(f"=== Stage 4: Self-Distillation Trace Collection (14B 8-bit): {args.num_samples} Samples ===")
    ...
    print(f"Loading {MODEL_ID} in 8-bit on {DEVICE}...")
