#!/usr/bin/env python3
"""
06_train_curriculum_lora.py
Curriculum LoRA fine-tuning for continuous latent recurrence:
...
Optimizer: AdamW + Cosine schedule, grad clipping 1.0, bfloat16.
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train LoRA curriculum for continuous latent recurrence.")
    parser.add_argument("--traces_file", type=str, default="data/traces_gsm8k_200.jsonl", help="Self-distilled traces path")
    parser.add_argument("--output_dir", type=str, default="checkpoints/lora_recurrent_1.5b", help="Output checkpoint directory")
...
def compute_sample_loss(model, tokenizer, sample, stage, num_latent_steps, scale_factor, max_target_len, device):
    """
    Computes end-to-end BPTT cross-entropy loss for a single sample.
    Stage 1: 3 latent steps replace initial think tokens; supervises remaining think + answer.
    Stage 2: 6 latent steps replace entire think block; supervises </think> + answer.
    """
    prompt_ids = tokenizer(sample["formatted_prompt"], return_tensors="pt", add_special_tokens=False).input_ids.to(device)
    
    if stage == 1:
        # Partial latent replacement:
    ...
    logits_0 = model.lm_head(curr_latent)  # [1, 1, vocab_size]
...
def main():
    ...
    print(f"=== EXP-06: Continuous Latent Recurrence LoRA Training ===")
    ...
    def train_stage(stage_num, num_epochs, recurrent_steps):
        ...
        # Baseline validation before stage training
        ...
        for epoch in range(num_epochs):
            ...
            for idx, sample in enumerate(train_samples):
                try:
                    ...
                    if torch.isnan(loss) or torch.isinf(loss):
                        print(f"  [Warning] NaN/Inf loss encountered at sample {idx}. Skipping.")
                        continue
            ...
            # Save checkpoint if best within stage
            ...
            if stage_num == 2 and val_loss < overall_best_stage2_val_loss:
                ...
                # Also save directly to output_dir so root always has the best Stage 2 model
