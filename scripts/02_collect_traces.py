...
Self-distillation data collector:
Generates natural reasoning traces (<think> ... </think> + final answer)
from deepseek-ai/DeepSeek-R1-Distill-Qwen-1.5B on GSM8K questions.
Saves parsed traces to data/traces_gsm8k_200.jsonl
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description="Collect reasoning traces from DeepSeek-R1-Distill-1.5B")
    parser.add_argument("--num_samples", type=int, default=200, help="Number of traces to collect")
    parser.add_argument("--batch_size", type=int, default=8, help="Batch size for parallel generation")
    parser.add_argument("--output_file", type=str, default="data/traces_gsm8k_200.jsonl", help="Output jsonl path")
    parser.add_argument("--max_new_tokens", type=int, default=768, help="Max tokens per generation")
    return parser.parse_args()

def main():
    args = parse_args()
    print(f"=== Self-Distillation Trace Collection: {args.num_samples} Samples ===")
    print(f"Device: {DEVICE} | Batch Size: {args.batch_size} | Target: {args.output_file}")
    ...
    with open(args.output_file, "w", encoding="utf-8") as out_f:
        for b_start in range(0, len(samples), args.batch_size):
            ...
            input_lens = [torch.sum(enc.attention_mask[i]).item() for i in range(len(batch_slice))]
