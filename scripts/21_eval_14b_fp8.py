...
"""
21_eval_14b_fp8.py
Comprehensive evaluation of DeepSeek-R1-Distill-Qwen-14B in FP8 (compressed-tensors dynamic FP8)
across 25 benchmark problems with UNCONSTRAINED max_new_tokens = 2048:
- Discrete Baseline CoT (max_baseline_tokens=2048)
- Fixed K=6 Recurrent LoRA (max_new_tokens=2048)
- Trigger A: Native </think> probability (tau=0.95) (max_new_tokens=2048)
- Trigger B: Geometric convergence (cos >= 0.992 or rel_L2 <= 0.10) (max_new_tokens=2048)
- Trigger C: Dynamic Hybrid (P >= 0.90 AND convergence) (max_new_tokens=2048)
- Trigger D: Auxiliary Probe Entropy Stabilization (|Delta H| <= 0.20 AND P >= 0.90) (max_new_tokens=2048)
- Think Harder: Native </think> logit suppression (steps 1..6) (max_new_tokens=2048)

Saves to data/eval_results_14b_fp8_2048.json.
Runs on cuda:1.
"""
...
OUTPUT_FILE = "data/eval_results_14b_fp8_2048.json"
...
def run_recurrent_step(
    ...
    with torch.no_grad():
        ...
        for step in range(1, target_max + 1):
            ...
            scaled_latent = (latent_in * scale_factor).to(model.dtype)
            step_out = model(
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate 14B FP8 model with max_new_tokens=2048")
    parser.add_argument("--rerun_discrete", action="store_true", help="Re-run discrete baseline in FP8 instead of reusing verified 2048-token discrete results")
    args = parser.parse_args()

    print(f"=== Evaluating 14B Model in FP8 (Discrete vs Recurrent) with max_new_tokens=2048 on {DEVICE} ===")
    
    tokenizer = AutoTokenizer.from_pretrained(MODEL_ID)
    think_end_id = tokenizer.encode("</think>", add_special_tokens=False)[-1]
    
    print(f"Loading FP8 base model {MODEL_ID} on {DEVICE}...")
    ...
    # Run dummy forward to trigger compressed_tensors weight decompression
    print("Running initial dummy forward pass on base_model to initialize compressed-tensors...")
    dummy_in = tokenizer("init", return_tensors="pt").to(DEVICE)
    with torch.no_grad():
        _ = base_model(**dummy_in)
    ...
    if not args.rerun_discrete and os.path.exists(PREV_RESULTS):
        print(f"Reusing verified 2048-token discrete baseline results from {PREV_RESULTS}...")
        with open(PREV_RESULTS, "r", encoding="utf-8") as f:
            prev_data = json.load(f)
        discrete_runs = prev_data["detailed_runs"]["discrete_baseline"]
        summary_dict["discrete_baseline"] = prev_data["summary"]["discrete_baseline"]
        detailed_dict["discrete_baseline"] = discrete_runs
        print(f"Loaded discrete baseline: Overall Acc {summary_dict['discrete_baseline']['overall_accuracy']}%, ThinkTime {summary_dict['discrete_baseline']['mean_think_time_ms']:.1f}ms")
    else:
        discrete_runs = []
        for idx, item in enumerate(eval_items):
            ...
            print(f"[{idx+1}/{len(eval_items)}] [{item['category']}] Discrete CoT (FP8): {q[:55]}... (GT: {gt_num or item['answer'][:15]})")
        ...
        detailed_dict["discrete_baseline"] = discrete_runs
        summary_dict["discrete_baseline"] = compute_stats(discrete_runs, "Discrete Baseline CoT (FP8)")
        
    # 2. LOAD PEFT LORA ADAPTER
    print("\n" + "="*90)
    print("LOADING PEFT LORA ADAPTER ON FP8 BASE...")
    print("="*90)
    ...
    conditions = [
        ("fixed_k6_recurrent", "Fixed K=6 Recurrent", {"k_mode": "fixed", "k_fixed": 6, "max_new_tokens": 2048}),
        ("trigger_a_logit", "Trigger A (Logit/P_think >= 0.95)", {"k_mode": "dynamic", "trigger_type": "trigger_a_logit", "tau_think": 0.95, "max_new_tokens": 2048}),
        ("trigger_b_conv", "Trigger B (Geometric Conv)", {"k_mode": "dynamic", "trigger_type": "trigger_b_conv", "tau_cos": 0.992, "tau_l2": 0.10, "max_new_tokens": 2048}),
        ("dynamic_hybrid", "Dynamic Hybrid (Trigger C)", {"k_mode": "dynamic", "trigger_type": "hybrid", "tau_think": 0.90, "tau_cos": 0.992, "tau_l2": 0.10, "max_new_tokens": 2048}),
        ("trigger_d_entropy", "Trigger D (Entropy Stabilized)", {"k_mode": "dynamic", "trigger_type": "entropy_stabilized", "tau_think": 0.90, "tau_entropy": 0.20, "max_new_tokens": 2048}),
        ("think_harder_suppress", "Think Harder (Logit Suppress steps 1..6)", {"k_mode": "dynamic", "trigger_type": "hybrid", "tau_think": 0.90, "suppress_think_steps": 6, "k_min": 8, "k_max": 16, "max_new_tokens": 2048})
    ]
    
    for cond_key, cond_name, cond_kwargs in conditions:
        print("\n" + "="*90)
        print(f"EVALUATING CONDITION: {cond_name} (max_new_tokens=2048, FP8)")
        print("="*90)
        cond_runs = []
        for idx, item in enumerate(eval_items):
            q = item["question"]
            ...
            print(f"[{idx+1}/{len(eval_items)}] [{item['category']}] -> Pred: {res['predicted_num']} | Correct: {is_correct} | Steps: {res['actual_steps']} ({res['halt_reason']}) | ThinkTime: {res['think_time_ms']:.1f}ms | AnsTok: {res['answer_tokens']}")
    ...
    print("\n" + "="*110)
    print("14B FP8 BENCHMARK EVALUATION SUMMARY TABLE (max_new_tokens=2048)")
    print("="*110)
