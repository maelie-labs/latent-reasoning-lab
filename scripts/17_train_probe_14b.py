#!/usr/bin/env python3
"""
17_train_probe_14b.py
Trains the auxiliary Thought-Stream Projection Probe for DeepSeek-R1-Distill-Qwen-14B.
Base model loaded in 8-bit on cuda:1 (RTX PRO 4500 32GB).
Maps intermediate continuous latent states h_t (in R^5120) to vocabulary space (152064).
Saves checkpoint to checkpoints/thought_probe_14b.pt.
"""
...
class TraceDataset(Dataset):
    def __init__(self, jsonl_path, max_samples=85):
        self.examples = []
...
def parse_args():
    ...
    parser.add_argument("--traces_file", type=str, default="data/traces_gsm8k_14b.jsonl")
    parser.add_argument("--save_path", type=str, default="checkpoints/thought_probe_14b.pt")
...
def main():
    args = parse_args()
    print("=== Stage 4: Training Auxiliary Thought-Stream Probe (14B) ===")
        ...
        for b_idx, (prompts, think_texts) in enumerate(dataloader):
            ...
            with torch.no_grad():
                ...
                for step in range(args.recurrent_steps):
                    ...
                    latents_per_step.append(curr_latent.to(torch.bfloat16))
            ...
            steps += 1
            if (b_idx + 1) % 15 == 0 or (b_idx + 1) == len(dataloader):
    ...
    print(f"14B Probe successfully saved to {args.save_path} in {total_time:.1f}s")
