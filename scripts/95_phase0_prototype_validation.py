#!/usr/bin/env python3
"""
scripts/95_phase0_prototype_validation.py
Phase 0 Prototype Validation Suite for the Register-Bundle Ladder.

Executes four rigorous checks on checkpoints/test_register_ladder_quick on cuda:1 (RTX PRO 4500):
1. Check A: Attention Split (Headers vs. Latents) across all 28 layers on 60 MATH L3-5 problems.
   - Decision Rule: Latents >= 10.0% -> CONTINUE, Latents < 5.0% -> TREAT AS TEXT-SUBGOAL.
2. Check C: 4-Way Matched Inference Ablation on 60 MATH L3-5 problems:
   - Cond 1: Full Ladder (3 headers + 24 latents)
   - Cond 2: Headers Only (3 headers + 0 latents; tests no compute)
   - Cond 3: Latents Only (generic filler headers + 24 latents; tests no text subgoals)
   - Cond 4: Headers + 24 Pause Tokens (3 headers + 24 filler tokens; near-FLOP matched discrete filler)
3. Check D: Multi-Bundle Latent Noise Sensitivity:
   - Evaluates divergence rate under norm-matched Gaussian noise across Bundle 1, Bundle 2, and Bundle 3 separately.
4. Check B: Latent-Only Donor Patching on N=94 valid counterfactual pairs:
   - Control 0 (Self-Patch Identity): Enforces >= 95.0% identity gate before reporting DeltaSteer.
...
def get_headers(sample, mode="full"):
...
def run_single_ladder_inference(
    ...
    return_bundle_latent=None, # bundle index whose final latent to return (e.g. 2)
    return_attention=False,
    max_ans_tokens=256
):
    """
    Unified batch-invariant (B=1) ladder inference using eager attention for 100% bitwise determinism.
    """
    ...
    with torch.inference_mode():
        ...
        for r, hdr_text in enumerate(headers):
            ...
            if condition in ["full", "latents_only"]:
                ...
                # Inject 8 pause tokens (filler delay tokens)
                ...
                # No latents, no pauses -> 0 positions
                pass
        ...
        # Seed token: first token predicted from trans_out logits
        ...
        attn_masses = None
        if return_attention:
            # Run 1 step with output_attentions=True
            ...
            # Decompose attention mass across all 28 layers
...
def main():
    parser = argparse.ArgumentParser(description="Phase 0 Prototype Validation")
    ...
    parser.add_argument("--only_check_b", action="store_true", help="Run only Check B (preserves A, C, D from existing output_file)")
    args = parser.parse_args()

    print("=" * 80)
    print("PHASE 0: PROTOTYPE VALIDATION SUITE")
    ...
    print("Pre-registered Decision Rule: Latents >= 10.0% -> CONTINUE | Latents < 5.0% -> TREAT AS TEXT-SUBGOAL")
    print("=" * 80)

    prompt_masses, header_masses, latent_masses, trans_masses = [], [], [], []
    for p in tqdm(math_l35, desc="Check A Attention"):
        hdrs = get_headers(p, mode="full")
        res = run_single_ladder_inference(model, tokenizer, p["question"], hdrs, args.device, condition="full", return_attention=True, max_ans_tokens=1)
        m = res["attn_masses"]
    ...
    for p in tqdm(math_l35, desc="Check C 4-Way Ablation"):
        ...
        q = p["question"]
        hdrs_full = get_headers(p, mode="full")
        hdrs_filler = get_headers(p, mode="generic_filler")

        # 1. Full Ladder
        res1 = run_single_ladder_inference(model, tokenizer, q, hdrs_full, args.device, condition="full", max_ans_tokens=256)
        c1 = check_correctness(res1["text"], gold)
        accs["full"] += c1
        clean_generations["full"].append(res1["text"])

        # 2. Headers Only (0 latents)
        res2 = run_single_ladder_inference(model, tokenizer, q, hdrs_full, args.device, condition="headers_only", max_ans_tokens=256)
        ...
        res3 = run_single_ladder_inference(model, tokenizer, q, hdrs_filler, args.device, condition="latents_only", max_ans_tokens=256)
        ...
        res4 = run_single_ladder_inference(model, tokenizer, q, hdrs_full, args.device, condition="headers_pause", max_ans_tokens=256)
    ...
    print(f"  2. Headers Only (3 Headers, 0 Latents)       : {pct_headers:5.2f}% ({int(accs['headers_only'])}/{n_math})")
    print(f"  3. Latents Only (Filler Headers + 24 Latents): {pct_latents:5.2f}% ({int(accs['latents_only'])}/{n_math})")
    print(f"  4. Headers + 24 Pause Tokens (Matched Filler): {pct_pause:5.2f}% ({int(accs['headers_pause'])}/{n_math})")
    ...
    for i, p in enumerate(tqdm(math_l35, desc="Check D Noise")):
        ...
        for b in [1, 2, 3]:
            res_noise = run_single_ladder_inference(model, tokenizer, p["question"], hdrs, args.device, condition="full", noise_bundle=b, max_ans_tokens=256)
    ...
    print(f"  * Baseline Reference  : v1.1 status quo was 73.40%")
    ...
    for p in tqdm(math_100, desc="Clean Extraction"):
        hdrs = get_headers(p, mode="full")
        res = run_single_ladder_inference(model, tokenizer, p["question"], hdrs, args.device, condition="full", return_bundle_latent=2, max_ans_tokens=256)
    ...
    for trial in tqdm(valid_trials, desc="Control 0"):
        r_idx = trial["r_idx"]
        prob_R = math_100[r_idx]
        hdrs_R = get_headers(prob_R, mode="full")
    ...
    print(f"\nPhase 0 Complete! Full results saved to {args.output_file}")
