#!/usr/bin/env python3
"""
scripts/31_verify_qwen3_4b.py
Architectural verification and norm-scaling factor derivation for Qwen/Qwen3-4B on GPU 1 (RTX PRO 4500 Blackwell 32GB).
Tests pure uncompressed bfloat16 loading, 32 Q heads / 8 KV heads (4:1 GQA),
...
def main():
    ...
    vram_before = torch.cuda.memory_allocated(device_id) / (1024**3)
    print(f"Initial VRAM Allocated on {DEVICE}: {vram_before:.2f} GB")
    ...
    vram_model = torch.cuda.memory_allocated(device_id) / (1024**3)
    ...
    peak_vram = torch.cuda.max_memory_allocated(device_id) / (1024**3)
    ...
    out_file = os.path.join(out_dir, "qwen3_4b_profiling_results.json")
